/**
 * Supervisor Screen — Dead Letter Queue (DLQ) management.
 *
 * Protected view: visible only to users with role === 'supervisor' | 'admin'.
  ...
  const handleRetry = useCallback((errorDoc: SyncErrorDoc) => {
    showConfirm(
      ...
      async () => {
        ...
        if (originalEvent) {
          // Bump updated_at so the RxDB replication engine detects the change and re-pushes
