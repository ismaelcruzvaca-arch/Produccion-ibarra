/**
 * Nhost Function: sync-plant-metadata
 * =====================================
 *
 * Syncs user_plants.plant_id → auth.users.metadata on INSERT/UPDATE.
 *
 * Called by:
 *   - Hasura Event Trigger (POST): standard Hasura event payload
 *   - Direct admin (PATCH): body { userId, plantId }
 *
 * Flow:
 *   1. Parse the inbound request (POST = Hasura event | PATCH = direct)
 *   2. Validate x-webhook-secret header
 *   3. PATCH the Nhost Auth Management API to update user metadata
...
/**
 * Extracts { userId, plantId } from the inbound request.
 * Supports two formats:
 *   - Hasura Event Trigger POST:  { event: { data: { new: { user_id, plant_id } } } }
 *   - Direct PATCH:              { userId, plantId }
 */
async function extractPayload(req: Request): Promise<SyncPayload> {
  const body = await req.json();

  // Direct PATCH format: { userId, plantId }
  if (req.method === 'PATCH' && body.userId && body.plantId) {
    return { userId: body.userId, plantId: body.plantId };
  }
  ...
  if (!event || !event.data || !event.data.new) {
    throw new HttpError(
      'Unrecognized payload format. Expected Hasura event { event: { data: { new: { user_id, plant_id } } } } ' +
      'or direct PATCH { userId, plantId }',
      400,
      ERR_VALIDATION,
    );
  }

  const { user_id, plant_id } = event.data.new;
  if (!user_id || !plant_id) {
    throw new HttpError(
      `Missing fields in event.data.new: user_id=${user_id}, plant_id=${plant_id}`,
      400,
      ERR_VALIDATION,
    );
  }

  return { userId: user_id, plantId: plant_id };
}
 ...
 */
async function syncPlantToMetadata(
  userId: string,
  plantId: string,
  adminSecret: string,
  backendUrl: string,
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (response.ok) {
        console.log(
          `[sync-plant-metadata] Updated metadata for user ${userId}: plant_id=${plantId}`,
        );
        return;
      }
      ...
      if (response.status === 404) {
        ...
        console.error(
          `[sync-plant-metadata] User ${userId} not found in auth.users. Skipping retry.`,
        );
        return;
      }
    } catch (err) {
    ...
    if (attempt < maxRetries) {
      ...
      console.warn(
        `[sync-plant-metadata] Attempt ${attempt}/${maxRetries} failed. Retrying in ${delayMs}ms...`,
        lastError?.message,
      );
  ...
  throw new HttpError(
    lastError?.message ?? `Failed to sync plant metadata after ${maxRetries} attempts`,
    502,
    ERR_UPSTREAM,
  );
}
...
function validateWebhookSecret(req: Request, expectedSecret: string): void {
  if (!expectedSecret) {
    ...
    console.warn(
      '[sync-plant-metadata] NHOST_WEBHOOK_SECRET not set — skipping validation',
    );
    return;
  }
  ...
  if (provided !== expectedSecret) {
    console.error(
      `[sync-plant-metadata] Invalid webhook secret (provided: ${provided ? '***' : 'none'})`,
    );
...
export default async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Parse the payload
    const { userId, plantId } = await extractPayload(req);
    ...
    await syncPlantToMetadata(userId, plantId, adminSecret, backendUrl);

    const elapsed = Date.now() - startTime;
    console.log(
      `[sync-plant-metadata] Completed in ${elapsed}ms for user ${userId}`,
    );
    ...
    console.error(`[sync-plant-metadata] ${status} Error after ${elapsed}ms: ${message}`);
